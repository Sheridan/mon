...
#define MON_DEFAULT_APPLICATION_INIT \
  MON_LOG_DBG("- Application init ------------"); \
  MON_ST_CONFIG->load(MON_CONF_FILE); \
  MON_CFOLDER *init_fldr = MON_ST_CONFIG->folder(MON_MODULE_NAME); \
  MON_ST_CONFIG->cd(init_fldr); \
  init_fldr = MON_ST_CONFIG->folder("logger"); \
  ...
  MON_LOG_DBG("- Application init done ------------"); \
